...
現在のHTMLで新設された`progress`や`meter`要素は経過や蓄積を表現する要素です。どれくらい過ぎたかや溜まったかは`value`属性を使って表現します。その`value`属性では0から1の間で指定するのですが、`min`属性や*特に`max`属性*を指定しておくことでその範囲を変更できます。
