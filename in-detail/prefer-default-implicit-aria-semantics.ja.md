...
しかし、`role`属性を使えば自由に要素の機能を変えられるわけではありません。多くの要素は限られたいくつかの機能の中から1つ指定するだけにとどまります。例えば`a`要素に`article`ロールは割り当てられません。中には仕様で定義されているデフォルトの機能以外に変更することはできない要素もあります。こういった変更できない*強いネイティブセマンティクス*を持つ要素では、事実上`role`属性を使うことはできません。
