...
HTML 4.01では`em`と`strong`要素はどちらも強調で、その度合いだけが違うものでした。現在のHTMLでは、強調は`em`要素になり、`strong`要素は用途が変わり、重要性や深刻さ、緊急性を意味するものになりました。警告や注意は深刻なもののため、良い例で挙げたように`strong`要素が適切になります。
