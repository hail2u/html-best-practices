...
Консистенция — залог удобного чтения.
...
Это также придает консистенцию.
...
### Не смешивайте `data-*`, Microdata и RDFa Lite атрибуты с обычными атрибутами
...
Некоторые элементы не нуждаются в ARIA-роли в HTML-документе, поэтому не указывайте их.
...
Удобочитаемая метка помогает людям выбрать подходящую таблицу стилей.
...
HTTP-заголовки должны задаваться сервером, будьте проще.
...
### Пропустите атрибут `type` для CSS
...
Элемент `address` предназначен для адреса электронной почты, аккаунта в социальной сети, адреса улицы, номера телефона или чего-то, с помощью чего с вами можно связаться.
...
Содержимое элемента `blockquote` — это цитата, а не символы.
...
### Не включайте атрибуцию непосредственно в элементе `blockquote`
...
### Пишите по одному элементу списка в строке
...
Иногда на маркер ссылается содержимое в ближней части. Если вы измените маркер с помощью атрибута `type`, то вы сможете безопасно ссылаться.
...
### Используйте атрибут `download` для скачивания ресурса

Это заставит браузеры скачать связанный ресурс в хранилище.
...
Эти подсказки помогают приложениям работать со связанными ресурсами.
...
Это серьезно. Поэтому элемент `strong` более уместен.
...
Семантика этих элементов слишком сложна для людей.
...
### Добавьте атрибут `title` к элементу `abbr`
...
Поддержка элемента `ruby` не реализована в современных браузерах.
...
Вложенность элемента `kbd` слишком сложна для людей.
...
Перенос строки должен быть необходим там, где используется элемент `br`.
...
Элемент `br` не предназначен для декоративного переноса строки, он предназначен для переноса строки в содержимом.
...
### Пропустите атрибут `alt`, если это возможно
...
Вспомогательный контент необходим для новых элементов в HTML.
...
### Оберните элемент управления формой элементом `label`
...
### По возможности пропустите атрибут `for`
...
С помощью соответствующего `type` браузер наделяет элемент `input` небольшими возможностями.
...
Если вводимый текст не соответствует атрибуту `pattern`, то значение атрибута `title` будет отображаться в качестве подсказки.
...
### Укажите протокол URL для внешних ресурсов
